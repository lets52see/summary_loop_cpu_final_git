...
parser.add_argument("--max_output_length", required=True, type=int,
...
parser.add_argument("--optim_every", type=int, default=4,
                    help="Optimize every x backprops. A multiplier to the true batch size.")
parser.add_argument("--device", type=str, default="cuda", help="cuda or cpu")
parser.add_argument('--fp16', action='store_true',
                    help="Whether to use 16-bit (mixed) precision (through NVIDIA apex) instead of 32-bit")
parser.add_argument('--starter_model', default="",
                    help="which model to start with. Leave empty string for random inizialitation")
...
model = GeneTransformer(tokenizer_type="gpt2", max_output_length=args.max_output_length, device=args.device,
                        bpe_model="")
...
def collate_func(documents):
    return [utils_misc.cut300(doc['article']) for doc in documents], [doc['highlights'] for doc in documents]
...
dl_train = DataLoader(dataset=d_train, batch_size=args.train_batch_size, sampler=RandomSampler(d_train),
...
if args.fp16:
    ...
    model.model, optimizer = amp.initialize(model.model, optimizer,
                                            opt_level="O2")  # For now O1. See details at https://nvidia.github.io/apex/amp.html
