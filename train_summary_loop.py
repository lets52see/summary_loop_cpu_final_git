...
parser.add_argument("--ckpt_every", type=int, default=120, help="If 0, checkpointing is not used. Otherwise, checkpointing is done very x seconds.")
...
def collate_func(inps):
    if ".db" in args.dataset_file:
        return [a['article'] for a in inps]
    else:
...
##sa
# coverage_model_file = os.path.join(models_folder, "bert_coverage_"+args.experiment +".bin")
...
##sa
# fluency_news_model_file = os.path.join(models_folder, "gpt2_"+ args.experiment +".bin")
...
for epi in range(n_epochs):
    ...
    for ib, documents in enumerate(dataloader):
        ...
        with torch.no_grad():
            argmax_summaries, argmax_end_idxs = summarizer.decode_batch(bodies, max_output_length=args.max_output_length, input_past=input_past,return_scores=True)
        T4 = time.time()
        ...
        if ckpt_every > 0 and len(total_score_history) > ckpt_lookback:
            ...
            if time.time()-time_ckpt > ckpt_every:
                ...
                if revert_ckpt:
                    ...
                    summarizer.model.load_state_dict(torch.load(ckpt_file, map_location=args.device))##sa
                    optimizer.load_state_dict(torch.load(ckpt_optimizer_file, map_location=args.device))##sa
                time_ckpt = time.time()
                print("==============================================================================")
