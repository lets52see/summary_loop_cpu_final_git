...
STOP_WORDS = set(
    ["'", ".", "!", "?", ",", '"', '-', 'we', 'our', 'you', 'he', 'him', 'she', 'her', 'it', "it's", 'its', 'they',
     'their', 'this', 'that', 'these', 'those', 'am', 'is', 'are', 'was', 'were', 'be', 'been', 'have', 'has', 'had',
     'do', 'does', 'did', 'a', 'an', 'the', 'and', 'or', 'as', 'of', 'at', 'by', 'to', 'not', 'so', "'s", "in", "for",
     "with", "on"])
...
class LengthPenalty:
    ...
    def score(self, summaries, bodies, bodies_tokenized=None, lengths=None, extra=None):
        ...
        scores = [1.0 if L > self.target_length else 1.0 - L / self.target_length for L in
                  lengths]  # This lets it go beyond for free
...
##sa
class LengthPenalty0:
    # Depending on how many words are used a large fraction of the last X summaries
    def __init__(self, target_length):
        self.target_length = float(target_length)
    ##sa
    def score(self, summaries, bodies, bodies_tokenized=None, lengths=[], extra=None):
        # In lengths, the number of tokens. Is -1 if the summary did not produce an END token, which will be maximum penalty, by design.
        # scores = [1.0-L/self.target_length for L in lengths]
        ##sa??
        for summary in summaries:
...
class RepeatPenalty:
    ...
    def score(self, summaries, bodies, bodies_tokenized=None, lengths=None, extra=None):
        ...
        for summary in summaries:
            ...
            if len(word_counts) > 0 and len(all_word_counts) > 0 and (
                    word_counts.most_common(1)[0][1] > N_1 or all_word_counts.most_common(1)[0][1] > N_2):
                # print(L, N_1, N_2)
...
if __name__ == "__main__":
    ...
    ##sa
    lenpen = LengthPenalty(20)
    print(lenpen.score([summary], [""], lengths=[4, 5]))

    lenpen = LengthPenalty(2)
    print(lenpen.score([summary], [""], lengths=[4, 5]))

    lenpen0 = LengthPenalty0(20)
    print(lenpen0.score([summary], [""]))
