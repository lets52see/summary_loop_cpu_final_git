...
class KeywordExtractor():
    ...
    def train(self):
        ...
        stop_indices = set([bert_w2i[w] for w in stop_words if w in bert_w2i]) ##sa self.
        dv = DictVectorizer()
        tt = TfidfTransformer()
...
class KeywordCoverage():
    ...
    def reload_model(self, model_file):
        ...
        print(self.model.load_state_dict(torch.load(model_file, map_location=map_location), strict=False))
...
##
    ...
    model_file = os.path.join(models_folder, "bert_coverage.bin")
    ...
    device = "cuda" if torch.cuda.is_available() else "cpu" ##sa
    kw_cov = KeywordCoverage(device, model_file=model_file, keyword_model_file=os.path.join(models_folder, "keyword_extractor.joblib"))
